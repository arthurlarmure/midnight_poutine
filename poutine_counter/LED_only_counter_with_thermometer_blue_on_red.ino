...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(22, STRIP_PIN, NEO_GRB + NEO_KHZ800);
...
void updateThermometer() {
  int goal = 50;
  int thermometerPixels = map(counter, 0, goal, 0, strip.numPixels());

  for (int i = 0; i < strip.numPixels(); i++) {
    if (i < thermometerPixels) {
      strip.setPixelColor(i, strip.Color(0, 0, 255)); // Blue when counter progresses
    } else {
      strip.setPixelColor(i, strip.Color(255, 0, 0)); // Red when counter goal is reached
    }
  }
